...
- Holistic Rules (8 sub-phases) cover 40 rules (rule_40 to rule_79):
  1. `#pragma once` in every .h file (suggestion) — rule_40
  2. Apache 2.0 copyright headers in all source files (suggestion) — rule_41
  3. No STL types where Thunder equivalents exist (warning) — rule_42
  4. ASSERT used correctly — on prerequisites, not return values (warning) — rule_43
  5. OOP registration order correct (violation, conditional) — rule_44
  6. Complete state reset in Deinitialize (violation) — rule_45
  7. Reverse-order cleanup in Deinitialize (suggestion) — rule_46
  8. Observer locking: lists protected when iterated or modified (violation) — rule_47
  9. AddRef/Release balance across plugin lifetime (violation) — rule_48
  10. CMake NAMESPACE variable used for target naming (suggestion) — rule_49
  11. Handlers must not block — dispatch to WorkerPool (violation) — rule_50
  12. No Activate/Deactivate calls from notification handlers (violation) — rule_51
  13. Shared member state protected by Core::CriticalSection (violation) — rule_52
  14. No framework callbacks (Notify, Submit, Register) called while holding _adminLock (violation) — rule_53
  15. WorkerPool jobs guard against post-Deinitialize execution (violation) — rule_54
  16. File descriptors and sockets wrapped in RAII (violation) — rule_55
  17. No unbounded memory growth in event-fed containers (violation) — rule_56
  18. Config validation failures return non-empty string from Initialize (violation) — rule_57
  19. interface->Register paired with interface->Unregister in Deinitialize (violation) — rule_58
  20. Handler registration order in Initialize/Deinitialize: acquire first, unregister before release (violation) — rule_59
  21. Use Core::ERROR_* named constants for failure codes, not raw integers (violation) — rule_60
  22. Input validation before use in JSON-RPC handlers (violation) — rule_61
  23. Event name strings as named constants; JSON payloads use typed JsonData::* classes (warning) — rule_62
  24. COM reference counting correctness: one AddRef per pointer, one Release per lifetime end (violation) — rule_63
  25. No hard inter-plugin dependencies; use dynamic interface acquisition (warning) — rule_64
  26. JSON-RPC handlers are re-entrant safe: shared state under lock (violation) — rule_65
  27. IPlugin::INotification callbacks must not block: delegate to WorkerPool (violation) — rule_66
  28. Lock scope minimized: no I/O or long operations while lock is held (violation) — rule_67
  29. Plugin threads joined/stopped in Deinitialize before releasing interfaces (violation) — rule_68
  30. Memory and allocation safety: no leaks, no use-after-free, no buffer overflows (warning) — rule_69
  31. Framework pointers (IShell*, acquired interfaces) not accessed from background threads after Deinitialize (violation) — rule_70
  32. Core::hresult return values from interface calls checked, not silently ignored (violation) — rule_71
  33. ASSERT used only for programmer invariants, not runtime conditions (warning) — rule_72
  34. Security: no sensitive data in logs, no shell commands from external input, paths sanitized (violation) — rule_73
  35. JSON-RPC input validated for type, bounds, enum range, and string length (violation) — rule_74
  36. Config completeness: all fields registered with Add(), config-based resources released (warning) — rule_75
  37. OOP error propagation: hresult from interface calls propagated; method names match conventions (warning) — rule_76
  38. Observer/notification inner classes declared as private nested members (suggestion) — rule_77
  39. No deprecated JSON-RPC APIs: no IDispatcher, no Announce-style events (violation) — rule_78
  40. All acquired interface pointers set to nullptr on all Deinitialize() exit paths (violation) — rule_79
