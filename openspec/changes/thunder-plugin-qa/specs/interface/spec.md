...
#### Scenario: Cross-platform Python setup
- GIVEN any OS (Windows, Mac, or Linux) with Python 3
- WHEN `python setup-prompts.py` is run from `ThunderTools/PluginQA/`
- THEN it auto-detects the VS Code settings.json location (platform-specific paths)
- AND also checks for VS Code Insiders variant
- AND creates a timestamped backup of existing settings
...
Because rules are loaded at runtime from YAML, updating existing rule definitions
(rewording, logic changes, or severity bumps) requires only editing the relevant YAML file.
No prompt file (`*.prompt.md`) needs to be changed for existing rule updates.
When rules are added or removed, the Quick Reference table in the corresponding
`.prompt.md` file must also be updated.
